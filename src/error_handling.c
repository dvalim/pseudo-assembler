...
int checkRegister(char r[]) {
    ...
    return (test != CONV__ERROR && test >= 0 && test <= 15);
}
...
void checkParsing(char tokens[][MAX_INPUT], int token_number, char line[]) {
    ...
    if(!if_label && (stringEquals(tokens[0], "DS") || stringEquals(tokens[0], "DC"))) error(INVALID_SYNTAX, line);
        ...
        if(stringToInt(tokens[0]) != CONV__ERROR) error(INVALID_SYNTAX, line);
        else if(token_number == 4) {
            if ((stringToInt(tokens[if_label+1]) == CONV__ERROR) || (!stringEquals(tokens[if_label+2], "INTEGER"))) 
        ...
        if(stringToInt(tokens[0]) != CONV__ERROR) error(INVALID_SYNTAX, line);
            ...
            if ((stringToInt(tokens[if_label+1]) == CONV__ERROR) || (!stringEquals(tokens[if_label+2], "INTEGER")) ||
            (stringToInt(tokens[if_label+3]) == CONV__ERROR)) 
                error(INVALID_SYNTAX, line);
        } else if(token_number == 4) {
            if(!stringEquals(tokens[if_label+1], "INTEGER") || stringToInt(tokens[if_label+2]) == CONV__ERROR)
                error(INVALID_SYNTAX, line);
        } else error(INVALID_SYNTAX, line);
...
void checkCommandExecution(command c) {
    ...
    if(test == CONV__ERROR && id2 == -1) error(VARIABLE_MISSING, line);
